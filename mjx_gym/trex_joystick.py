...
def default_config() -> config_dict.ConfigDict:
    ...
    config.reward_config.scales = config_dict.create(
        ...
        getup_torso_height=0.0,
        getup_foot_support=0.0,
        getup_foot_balance=0.0,
        getup_foot_placement=0.0,
...
def joystick_config() -> config_dict.ConfigDict:
    ...
    config.episode_length = 1000
    config.reset_standing_prob = 1.0
    config.command_config.forward_min = 0.25
    config.command_config.forward_max = 0.8
    config.command_config.high_speed_min = 0.8
    config.command_config.high_speed_prob = 0.0
    config.command_config.turn_max = 0.25
    config.command_config.zero_prob = 0.05
    config.command_config.turn_zero_prob = 0.0
    config.apply_gait_prior_action = True
    config.gait_frequency_min = 0.9
    config.gait_frequency_per_mps = 0.25
    config.gait_frequency_max = 1.6
    config.gait_swing_height = 0.14
    config.gait_prior_scale = 0.45
    config.walk_action_residual_scale = [0.25] * 8 + [0.5, 0.5]
    ...
    config.reward_config.scales.tracking_ang_vel = 6.0
    config.reward_config.scales.forward_progress = 1.5
    config.reward_config.scales.forward_speed_deficit = -3.0
    config.reward_config.scales.orientation = -4.0
    config.reward_config.scales.base_height = -1.0
    config.reward_config.scales.low_torso_height = -10.0
    config.reward_config.scales.feet_phase = 0.5
    ...
    def _get_reward(
        ...
        return {
            ...
            * self._cost_forward_speed_deficit(info["command"], local_linvel),
            ...
            "getup_foot_placement": orientation
            * self._reward_foot_placement(data),
            "getup_standing_pose": orientation * self._reward_standing_pose(data.qpos),
            "getup_base_lin_vel": orientation
            * height
