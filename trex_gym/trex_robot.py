...
class BulletRobotBase(object):
    ...
    * set_actions
    ...
    def reset(self, reload_urdf=False):
        ...
        if reload_urdf or self.body_handle is None:
            ...
            if self.world_origin_xyz:
                arguments['basePosition'] = self.world_origin_xyz
            if self.world_origin_rpy:
                arguments['baseOrientation'] = self.client.getQuaternionFromEuler(self.world_origin_rpy)
            if self.fixed_base:
    ...
    def zero_joint_state(self):
        ...
        """
        for joint_index in range(self.client.getNumJoints(self.body_handle)):
            self.client.resetJointState(self.body_handle, joint_index, targetValue=0., targetVelocity=0.)
...
class TrexRobot(BulletRobotBase):
    ...
    _BASE_LINK_NAME = 'vertebrae_sacral_link'

    def __init__(self, pybullet_client, urdf_path, world_origin_xyz=None, world_origin_rpy=None):
        """Holds the bullet body handle.
    ...
    def get_base_position(self):
        ...
        """
        link_info = self.get_link_state(self._base_link_index)
        return link_info['linkWorldPosition']
    ...
    def get_observation_limits(self):
        ...
        :return: a tuple of (lower_limit, upper_limit) giving the acceptable observation limits.
        """
    ...
    def get_action_limits(self):
        ...
        :return: a tuple of (lower_limit, upper_limit) giving the acceptable action limits.
        """
        num_joints = len(self._revolute_joint_indices)
        lower_limit, upper_limit = self._get_joint_limits()
